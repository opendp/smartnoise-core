...
pub fn simple_geometric_mechanism(data: &ArrayD<i64>, epsilon: &f64, count_min: &i64, count_max: &i64, enforce_constant_time: &bool) -> ArrayD<i64> {
    ...
    let mut data_vec: Vec<i64> = data.clone().into_dimensionality::<Ix1>().unwrap().to_vec();
