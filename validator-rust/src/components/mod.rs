        ...
        match release {
            Some(release) => match release {
                ...
                Value::Array(value) => match value {
                    Array::F64(array) => (0..value.num_columns()? as usize)
                        .map(|index| ndarray_to_name(get_ith_release(array, &index)?))
                        .collect::<Result<Vec<String>>>(),
                    Array::I64(array) => (0..value.num_columns()? as usize)
                        .map(|index| ndarray_to_name(get_ith_release(array, &index)?))
                        .collect::<Result<Vec<String>>>(),
                    Array::Bool(array) => (0..value.num_columns()? as usize)
                        .map(|index| ndarray_to_name(get_ith_release(array, &index)?))
                        .collect::<Result<Vec<String>>>(),
                    Array::Str(array) => (0..value.num_columns()? as usize)
                        .map(|index| ndarray_to_name(get_ith_release(array, &index)?))
                        .collect::<Result<Vec<String>>>()
                }
            },
...
fn ndarray_to_name<T: ToString>(array: ArrayD<T>) -> Result<String> {
    match array.ndim() {
        0 => match array.first() {
